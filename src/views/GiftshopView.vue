<template>
  <div>
    <section class="header">
      <img
        id="header-shape"
        src="@/assets/GiftshopView/header-shape.png"
        alt=""
      />
      <div style="margin-left: 60px">
        <h1 style="font-size: 50px; margin-top: 50px">
          Leva um pedaçinho <br />do nosso circo para casa!
        </h1>
        <br />
        <p style="font-size: 20px">
          Encontre o presente perfeito e leve um pedacinho do nosso circo
          consigo!
        </p>
      </div>
    ...
    <section class="catalog" style="margin-left: 60px; margin-right: 60px">
      <div class="product-card" v-for="product in productStore.products">
        <img
          src="@/assets/GiftshopView/card-background.svg"
          alt=""
          id="card-background-img"
        />
        <img id="card-product-img" :src="product.image" alt="" />
        <img
          v-if="product.exclusive"
          src="@/assets/GiftshopView/exclusive2.svg"
          alt=""
          id="exclusive-icon"
        />
        ...
        <div class="card-add-product">
          <button id="card-btn">
            <RouterLink
              id="card-btn-routerlink"
              :to="{ name: 'gift', params: { id: product.id } }"
            >
          ...
          <button id="card-cart-icon" @click="addToCart(product)">
            <img
              src="@/assets/GiftshopView/cart-icon.svg"
...
export default {
  ...
  methods: {
    addToCart(product) {
      ...
      let newProduct = {
        units: 1,
        product: this.productStore.products.find(
          (giftshopProduct) => giftshopProduct.id == product.id,
        ),
        date: "",
      };
